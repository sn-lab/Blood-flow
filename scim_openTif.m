...
[filePath,fileStem,fileExt] = fileparts(which(fileName));
...
if forceFlat
    Aout = zeros(numLines,numPixels,length(selChans)*numSelections,'uint16');
elseif forceFour
    Aout = zeros(numLines,numPixels,4,numSelections,'uint16');
elseif forceRGB
    Aout = zeros(numLines,numPixels,3,numSelections,'uint16');
elseif forceCell
    Aout = cell(1,maxNumChans);
    for i=1:length(savedChans)
        if ismember(savedChans(i),selChans)
            Aout{savedChans(i)} = zeros(numLines,numPixels,numSelections,'uint16');
        end
    end
else
    Aout = zeros(numLines,numPixels,length(selChans),numSelections,'uint16');
...
try
    ...
    % NOTE: This would have been a preferable approach syntactically, but it degraded performance considerably
    %if verLessThan('matlab','7.8')
    %     if true
    %         imageRead = @imread;
    %     else
    %         imageRead = @(file,idx)imread(file,idx,'Info',info);
    %     end
    useImreadInfo = ~verLessThan('matlab','7.8');
    ...
    for i=1:length(selection)
        if ishandle(h)
            waitbar(i/length(selection),h, ['Loading ' selectionStr ' Number ' num2str(selection(i))]);
        end
...
if forceRGB   
    ...
    if ~useLUT
        Aout = Aout * (intmax('uint16')/max(Aout(:))); %this allows renormalization, without ever converting to double
    ...
    switch target
        ...
        case 'mip'
            ...
            if forceRGB
                ...
                h = waitbar(0,'Writing Output Image...', 'Name', 'Writing TIF Image', 'Pointer', 'watch');
                try
                    for i=1:length(selChans)
